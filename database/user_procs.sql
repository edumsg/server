...
CREATE OR REPLACE FUNCTION get_tweets(user_id integer) --gets user tweets and retweets
RETURNS refcursor AS $$
DECLARE cursor refcursor := 'cur';
  BEGIN
    OPEN cursor FOR
...
-- JAVA / JSON DONE
CREATE OR REPLACE FUNCTION get_feeds(user_id integer) --gets timeline of user
  RETURNS refcursor AS $$
DECLARE cursor refcursor := 'cur';
BEGIN
  OPEN cursor FOR
