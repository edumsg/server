...
<h2>Queues:<c:if test="${null != param.QueueFilter && param.QueueFilter != ''}"> (filter="<c:out
        value='${param.QueueFilter}'/>")</c:if></h2>
        ...
        <c:if test="${param.QueueFilter == '' || fn:containsIgnoreCase(row.name, param.QueueFilter)}">
            ...
            <tr>
                <td><a href="<c:url value="browse.jsp">
                        <c:param name="JMSDestination" value="${row.name}" /></c:url>"><form:tooltip text="${row.name}"
                                                                                                     length="50"/></a>
                </td>
                ...
                <td>
                   ...
                   <c:param name="JMSDestination" value="${row.name}" /></c:url>">Browse</a>
                    ...
                    <a href="queueBrowse/<form:escape text="${row.name}" />?view=rss&feedType=atom_1.0"
                       title="Atom 1.0"><img src="images/feed_atom.png"/></a>
                    <a href="queueBrowse/<form:escape text="${row.name}" />?view=rss&feedType=rss_2.0"
                       title="RSS 2.0"><img src="images/feed_rss.png"/></a>
                </td>
                <td>
