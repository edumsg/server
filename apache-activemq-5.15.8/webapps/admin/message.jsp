            ...
            <tr>
                <td class="layout" valign="top">
                    <table id="header" class="sortable autostripe">
                        ...
                        <tr>
                            <td class="label" title="The ID used to correlate messages together in a conversation">
                                Correlation ID
                            </td>
            ...
            <tr>
                <td class="layout" colspan="2">
                    <table id="body" width="100%">
                        ...
                        <tr>
                            <td colspan="2"><a
                                    href="<c:out value='deleteMessage.action?JMSDestination=${requestContext.messageQuery.JMSDestination}&messageId=${row.JMSMessageID}&secret=${sessionScope["secret"]}' />"
                                    onclick="return confirm('Are you sure you want to delete the message?')">Delete</a>
                            </td>
                        </tr>
                        ...
                        <tr class="odd">
                            <td>
                                <a href="<c:out value="javascript:confirmAction('queue', 'copyMessage.action?destination=%target%&JMSDestination=${requestContext.messageQuery.JMSDestination}&messageId=${row.JMSMessageID}&JMSDestinationType=queue&secret=${sessionScope['secret']}"/>')">Copy</a>
                            </td>
                            <td rowspan="2">
                                <select id="queue">
                                    ...
                                    <c:forEach items="${requestContext.brokerQuery.queues}" var="queues">
                                        <c:if test="${queues.name != requestContext.messageQuery.JMSDestination}">
                                            <option value="<c:out value="${queues.name}" />"><form:short
                                                    text="${queues.name}" length="80"/></option>
                                        </c:if>
                                    </c:forEach>
                                </select>
                            </td>
                        ...
                        <tr class="odd">
                            <td>
                                <a href="<c:out value="javascript:confirmAction('queue', 'moveMessage.action?destination=%target%&JMSDestination=${requestContext.messageQuery.JMSDestination}&messageId=${row.JMSMessageID}&JMSDestinationType=queue&secret=${sessionScope['secret']}"/>')"
                                >Move</a></td>
                        </tr>
                        </tbody>
                    </table>
                ...
                <td class="layout" colspan="2">
                    <table id="body" width="100%">
                        ...
                        <tr>
                            <td>
                                <div class="message">
                                    <pre class="prettyprint"><c:out value="${requestContext.messageQuery.body}"/></pre>
                                </div>
                            </td>
                        </tr>
                        </tbody>
